...
class PlasticSCMPreferences(bpy.types.AddonPreferences):
    ...
    def __update_cm_path(self, context):
        self.existing_cm_path = os.path.isfile(self.plastic_cm_path)

        client.set_cm_location(self.plastic_cm_path)
        client.refresh_status()

    plastic_cm_path: StringProperty(
        name='Path to the PlasticSCM cm executable',
        subtype='FILE_PATH',
    ...
    popup_width: IntProperty(
        name='Popup width',
        description='Width to use for the PlasticSCM popup menu',
        min=300,
        max=400,
        default=300
    )
    ...
    def draw(self, context):
        ...
        col = row.column()
        col.scale_x=1.2
        col.prop(self, 'plastic_cm_path', text='Path to cm', translate=False)
        ...
        if not self.existing_cm_path:
            row = layout.row()
            row.label(text='File path "' + self.plastic_cm_path + '" doesn\'t exist', icon='ERROR', translate=False)

def register():
    bpy.utils.register_class(PlasticSCMPreferences)

def unregister():
    bpy.utils.unregister_class(PlasticSCMPreferences)
